"""
完全数とは, その数の真の約数の和がそれ自身と一致する数のことである. たとえば, 28の真の約数の和は,
1 + 2 + 4 + 7 + 14 = 28 であるので, 28は完全数である.
...
for m in range(1, m + 1):
    if m < sum(divisorlist(m)):
        abundantnumberlist.append(m)

for n in range(1, m + 1):
    for i in abundantnumberlist:
        if (n - i) in abundantnumberlist:
            break
        if (n - i) > 0:
            continue
        else:
